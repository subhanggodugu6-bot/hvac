...
export const KPIGrid: React.FC<{
  ...
  <div className={className}>
    {items.map((kpi) => {
      ...
      const pending = loading && missing;
      const placeholder = pending ? loadingText : emptyText;
      return (
        <div key={kpi.label} className="kpi-tile">
          ...
          <div
            className={`mt-3 min-h-[1.7rem] text-[1.65rem] font-bold tracking-tight leading-none ${
              missing
                ? pending
                  ? 'text-slate-400 text-sm font-semibold animate-pulse'
                  : 'text-amber-600 text-sm font-semibold'
                : 'text-slate-900'
            }`}
          >
            {missing ? placeholder : kpi.value}
          </div>
          <div className="text-[11px] text-slate-600 mt-2 truncate">
            {missing ? (pending ? 'Waking API…' : '') : kpi.detail || kpi.unit || ''}
          </div>
